<?php
  // HTML class
  $classes = classNames( "{$type} {$field['class']}", $visibilityClasses);
  ...
  foreach ($field['galleries'] as  $t) {
      ...
      foreach ($portfolio_tags as $key => $portfolio_tag) {
          ...
          if(!in_array($portfolio_tag, $tags)){
              $tags[] = $portfolio_tag;
          }
      }
  }
    ...
    <div class="qx-fg-items">
      <?php foreach( $items as $gallery ):?>
        <?php foreach( $gallery as $item ):?>
          ...
          <div class="qx-col-md-<?php echo 12/$field['columns']?> qx-col-sm-6 item-padding qx-fg-item p-0 m-0<?php echo $itemtag ?> <?php echo $animation_class . $hover_animation ?>" data-wow-delay="<?php echo $animation_delay?>s">
            <div class="qx-fg-wrap">
              <figure class="qx-overlay qx-overlay-hover">
                ...
                <?php if($field['lightbox_enabled']):?>
                    ...
                    <div class="qx-overlay-panel qx-overlay-fade qx-flex qx-flex-center qx-flex-middle qx-text-center">
                      ...
                      <div class="qx-fg-content">
                        <?php if($field['title_enabled']):?>
                          <h4 class="qx-fg-title"><?php echo $item['title']?></h4>
                        <?php endif;?>
                  ...
                  <?php else:?>    
                    <div class="qx-overlay-panel qx-overlay-icon qx-overlay-fade">
						<?php if($field['title_enabled'] OR $field['description_enabled']):?>
						<a class="text-white mt-5" target="_blank" href="<?php echo '//'.$item['link']?>">
							<div class="qx-fg-content">
							  <?php if($field['title_enabled']):?>
								<h4 class="qx-fg-title"><?php echo $item['title']?></h4>
							  <?php endif;?>
